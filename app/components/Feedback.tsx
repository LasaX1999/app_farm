    ...
    <div className="flex items-center justify-center h-screen">
      <motion.div
        ...
        <span className="flex items-center justify-center gap-3 pl-4 pr-2">
          <div className="text-sm text-black">Like our service?</div>
              ...
              <button
                ...
                className={twMerge(
                  happiness === e.happiness
                    ? 'bg-blue-100 stroke-blue-500'
                    : 'stroke-neutral-500',
                  'flex h-8 w-8 items-center justify-center rounded-full transition-all hover:bg-blue-100 hover:stroke-blue-500'
                )}
                key={e.happiness}>
                {e.emoji}
              </button>
          ...
          <AnimatePresence>
            {!isSubmitted ? (
              <motion.span exit={{ opacity: 0 }} initial={{ opacity: 1 }}>
                <textarea
                  ref={textRef}
                  placeholder="Your app is awesome!"
                  className="min-h-32 w-full resize-none rounded-md border bg-white p-2 text-sm placeholder-neutral-400 focus:border-neutral-400 focus:outline-0"
                />
                <div className="flex h-fit w-full justify-end">
                  <button
                    onClick={() => submitFeedback(happiness!, textRef.current!.value || '')}
                    className={twMerge(
                      'mt-1 flex h-9 items-center justify-center rounded-md border bg-blue-500 px-2 text-sm text-white',
              ...
              <motion.div
                ...
                <motion.div
                  variants={item}
                  className="flex h-8 min-h-8 w-8 min-w-8 items-center justify-center rounded-full bg-blue-500">
